...
const ProjectDetails = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center
            ...
            backdrop-blur-sm">
            <motion.div
                className="relative w-[92vw] max-w-xl md:max-w-2xl border shadow-sm rounded-2xl bg-gradient-to-l from-midnight to-navy border-white/10 max-h-[80vh] overflow-auto"
            ...
            >
                ...
                <button onClick={closeModel} className="absolute p-2 rounded-sm top-5 right-5
                    ...
                    <img src="assets/close.svg" className="w-6 h-6" 
                        alt="button to close project details" />
                </button>
                        ...
                        <div className="flex gap-3">
                            {tags.map((tag) => (
                                <img key={tag.id} src={tag.path} alt={tag.name} 
                                    className="rounded-lg size-10 hover-anmation"    
                                />
                            ))}
                        </div>
                            <a href={href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-medium hover-animation">
                                View Project
                                <img src="/assets/arrow-up.svg" alt="Arrow up icon" className="size-4" />
                            </a>
                    </div>
                </div>
